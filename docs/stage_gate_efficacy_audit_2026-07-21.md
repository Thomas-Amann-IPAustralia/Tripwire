...
- **Historical backfill (§4) is now possible.** `src/backfill_scores.py` (run via the `backfill_scores.yml` `workflow_dispatch` job) replays Stages 5–6 for past **webpage** changes — the raw unified diff is committed per run, so `normalised_diff` is reconstructed faithfully — rescores every Stage-4 candidate against the current corpus, and joins each confirmed page to its existing LLM verdict. Results go to the new `score_backfill` table plus a calibration report, **without modifying** the historical `pipeline_runs` rows. FRL/RSS runs are recorded as skipped because their change documents are not retained per run. Scores are computed against the current corpus, so recent runs are the most faithful.
